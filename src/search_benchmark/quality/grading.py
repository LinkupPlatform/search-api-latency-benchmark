"""SimpleQA Verified grading prompt and metric calculation.

The prompt is the paper's official autorater prompt, as reproduced by
inspect-evals:
https://github.com/UKGovernmentBEIS/inspect_evals/tree/main/src/inspect_evals/simpleqa
"""
...
Return a JSON object with exactly one field named "grade", whose value is one of
"CORRECT", "INCORRECT", or "NOT_ATTEMPTED".
""".strip()


def calculate_metrics(grades: list[str]) -> dict[str, float | int]:
    ...
    correct_given_attempted = correct / attempted if attempted else 0.0
    denominator = correct_rate + correct_given_attempted
    f_score = (
        2 * correct_rate * correct_given_attempted / denominator if denominator else 0.0
    )
    return {
        "total": total,
        "correct": correct,
